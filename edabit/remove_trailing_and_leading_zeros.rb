...
# Sedrick's
# Split string by (.)
# □ capture return value in var
# □ check if the second ele includes all zero's
#     □ if true:
#       ﹥ convert to string
#       ﹥ to_i
#       ﹥ return
#    □ check if the second var includes any digits [1-9]
#     □ if false
#       ﹥ convert to string
#       ﹥ to_f
#       ﹥ return
# ▣ everything else join & convert to integer
def remove_leading_trailing(str)
  var = str.split(".")

  if var.last.chars.all? { |char| char == "0" } # algo says if true, convert to string, but convert what? the first elem or the second? I assume it's the first
    # why algo says convert to string?  It already is a string...
    return var.first
  end

  unless var.last.chars.any? { |char| char =~ /[1-9]/}
    # why algo says convert to string?  It already is a string...
    return var.last.char.to_f
  end

  var.join # we don't need to convert to an integer, str needs to be returned
end
