...
# in the array are smaller than it. When counting numbers, only count
# unique values. That is, if a given number occurs multiple times in
# the array, it should only be counted onc
#
# Problem
# =======
# -method takes an array
# -method returns for each number how many numbers are smaller
# -onlly unique nummbers are counted so if a nnum occurs multliple timmes, we onnly count it once
# -if all nnnums are samme, return 0 for each nuber
# -if 1 numm in the arg array, return 0
#
#
# Examples
# ========
...
# --for each number select the numbers tht are smaller from the array of unique values, count them and store the result in resullt variable
# -RETURN result
def smaller_numbers_than_current(arr)
  result = []

  arr.each do |number|
    result << arr.uniq.find_all { |n| n < number }.size
  end
