...
def str_expansion(str)
  ...
  chunks = str.chars.chunk_while { |_i, j| j.to_i == 0 }.to_a
...
# A:
# -define number-strings constant `NUMERICS`
# -define the `str_expansion` method
# -INIT `result`
# -INIT `multiplier`
#
# -ITERATE over str argument
# -for each character:
# --if it belongs to `NUMERICS`, set multiplier to this character
# --else multiply character by multiplier and append the result to `result`
NUMERICS = ('0'..'9')
def str_expansion(str)
  result = ''
  multiplier = 1
  ...
  str.each_char do |char|
    case NUMERICS.include?(char)
    when true  then multiplier = char.to_i
    when false then result << char * multiplier
    end
  end
...
# -ITERATE over str argument
# -for each character:
# --if it is a number-string, SET multiplier to this character
# --else multiply character by multiplier and append the result to `result`
NUMBER_STRINGS = ('0'..'9')
def str_expansion(str)
  result = ''
  ...
  counter = 0
  loop do
    if NUMBER_STRINGS.include?(str[counter])
      multiplier = str[counter].to_i
    else
      result << str[counter] * multiplier
    end
    counter += 1
...
def str_expansion(str)
  ...
  loop do
    ...
    if NUMBER_STRINGS.include?(current_character)
      multiplier = current_character.to_i
    else
      result << current_character * multiplier
    end
    counter += 1
    break if counter > str.size - 1
  end
...
def str_expansion(str)
  ...
  str.chars.each do |char|
    if NUMBER_STRINGS.include?(char)
